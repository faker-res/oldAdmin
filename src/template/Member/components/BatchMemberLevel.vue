<template>
  <div class="batchMemberLevel" style="display:inline-block">
    <Button type="primary" @click="SearchRest">修改会员等级</Button>
    <Modal class-name="vertical-center-modal" title="会员查询" v-model="showBatchModal" width="68%">
      <Form :model="searchParams" inline :label-width="50">
        <FormItem label="站点">
      ...
      <Row style="margin:10px 0">
        ...
        <i-col span="3">
          <LevelSelect v-show="memberLevel" ref="LevelSelect" :LevelSelectConf="LevelSelectOfMember" v-on:listenLevelChange="levelChange"></LevelSelect>
        </i-col>
        <i-col span="3" v-show="memberLevel">
          <Button @click="batchModifyLevel" type="success">确定</Button>
          <Button @click="cancelBatch" type="error">取消</Button>
        </i-col>
        <i-col v-show="memberLock" span="3">
          <RadioGroup v-model="batchLockForm.lock">
            <Radio label="yes">锁定</Radio>
            <Radio label="no">解锁</Radio>
          </RadioGroup>
        </i-col>
        <i-col span="3" v-show="memberLock">
      ...
      Membertable: {
        ...
        TableColumn: [
          ...
          },
          {
            ...
            render: (h, params) => {
              let text = this.filterMemberTableDataVal(
                params.row.along_finance_statistics_member,
                "offlineDepositNumber"
              );
              return h('span',text);
            }
          },
          ...
          },
          {
            ...
            minWidth: 150,
            render: (h, params) => {
              if (params.row.along_finance_statistics_member) {
                let text = this.$moment
                  .unix(
                    params.row.along_finance_statistics_member.statistics_time
                  )
